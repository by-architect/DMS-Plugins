...
// This file is deliberately free of Matrix: it is the framing layer, and exists
// unchanged in every bridge. Everything service-specific lives in bridge.go /
// sync.go / messages.go.
...
type chatObj struct {
	...
	// Subject is the room topic. The contract carries it for mail-shaped
	// providers; a Matrix topic is the same idea.
	Subject string `json:"subject,omitempty"`
	// Handles are the reachable identifiers for this conversation -- for Matrix,
	// the other person's user id and any published room alias. The room id is
	// unguessable, so without these a room could only be found by name.
...
type messageObj struct {
	...
	// BodyHTML is the formatted body. Matrix sends HTML alongside the plain
	// text, so the host can render it without the bridge flattening it first.
