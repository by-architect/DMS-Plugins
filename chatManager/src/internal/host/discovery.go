...
// discoverProviders scans for type "chat" plugins.
//
// A user plugin shadows a system one with the same id, matching how the shell
// resolves plugins.
//
// Providers are looked for in a directory of their own as well as among the
// shell's plugins. A chat provider has no QML and no surface the shell can
// draw, so stock DMS rejects its manifest as invalid and logs about it on every
// scan; keeping providers out of the plugins directory avoids that noise
// entirely. The plugins directory is still read so existing installs, where
// providers were symlinked in beside ordinary plugins, keep working untouched.
func discoverProviders(userDir string) []Provider {
	found := map[string]Provider{}

	// System first, so user plugins overwrite them.
	for _, root := range []string{systemPluginsDir, userProviderDir(), userDir} {
		if root == "" {
			continue
		}
...
// userProviderDir is where a chat provider belongs: alongside the shell's
// plugins rather than inside them.
func userProviderDir() string {
	configDir, err := os.UserConfigDir()
	if err != nil {
		return ""
	}
	return filepath.Join(configDir, "DankMaterialShell", "chat-providers")
}
